...
(function() {
  ...
  Rocklib.prototype.getScriptBody = function(src, attr) {
    let scriptBody = src.getAttribute(attr, src.parseTreeNode.text || "");

    if (src.parseTreeNode.text) {
      scriptBody = src.parseTreeNode.text;
    } else if (src.parseTreeNode.children) {
      let kids = src.parseTreeNode.children;
      for (let k in kids) {
        let kk = kids[k];
        switch (kk.type) {
  ...
  Rocklib.prototype.getOptions = function(src, tag, options) {
    try {
      ...
      // [tag-xxx]
      let tt = src.getVariable("currentTiddler");
      if (tt) {
        let t = src.wiki.getTiddler(tt);
        if (t) {
          ...
          for (let f in t.fields) {
            let fi = f.indexOf(tag);
            if (fi != 0) continue;
            let k = f.substring(tag.length + 1);
            let cap = k.indexOf(".");
            while (cap > -1) {
              if (k.length < cap + 2) break;
              k =
                k.substring(0, cap) +
                k.charAt(cap + 1).toUpperCase() +
                k.substring(cap + 2);
              cap = k.indexOf(".");
            }
            let v = t.fields[f];
      ...
      // for the object
      for (let att in src.attributes) {
        let attval = src.getAttribute(att);
        // allow for data from named tiddlers
        if ($tw.wiki.tiddlerExists(attval)) {
          let data = $tw.wiki.getTiddlerData(attval);
          options[att] = data;
        } else {
          // try as JSON
          try {
  ...
  Rocklib.prototype.getCanvas = function(src, tag, type) {
    ...
    let height = src.getAttribute("height");
    let width = src.getAttribute("width");

    let canvas = src.document.createElement(type);
    let style = "";
    if (type === "div") {
  ...
  Rocklib.prototype.nextColor = function(s, v) {
    ...
    this.hue %= 1;
    let rgb = this.hsv_to_rgb(this.hue, s, v);
